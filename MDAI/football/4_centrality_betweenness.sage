...
logging.debug("Betweenness centrality");
...
def getData(graphs):
    ...
    for i in range(len(graphs)):
        if not(graphs[i] == []):
            g = graphs[i][0];
            res.append(dict_to_list(g.centrality_betweenness()));
        if graphs[i] == []:
            res.append([]);
    
    return res;

logging.debug("RP betweenness centrality..." );
rp_bc = getData(rp_graphs);

logging.debug("GGA betweenness centrality..." );
...
figure = (fig_rp + fig_rs);
figure.axes_labels(['k','Betweenness centrality RMS']);
figure.set_legend_options(loc=0);
figure.set_legend_options(font_size=tamano_fuente_legend);
figure.fontsize(tamano_fuente);
figure.save(subdir_results +"result_BC_RMS.png", frame=false);
...
figure.set_legend_options(font_size=tamano_fuente_legend);
figure.fontsize(tamano_fuente);
figure.save(subdir_results +"result_BC_RMS-color.png", frame=false);

logging.debug("[DONE]Betweenness centrality");
