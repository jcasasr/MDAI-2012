...
for i in range(len(gga_graphs)):
    if not(gga_graphs[i] == []):
        asd.append(H1_analysis(gga_graphs[i][0]));
    if gga_graphs[i] == []:
        asd.append([]);
...
# write to file
f1 = open(subdir_results +"H1-GGA.log","w");
