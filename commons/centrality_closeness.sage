...
def getCCValues(G, name):
    ...
    for i in range(len(G)):
        ...
        for j in range(len(G[i])):
            ...
            save(valores_TMP, subdir_tmp_details + name +"-CC-"+str(i)+"-"+str(j));
            
        valores.append(valores_TMP);
        save(valores_TMP, subdir_tmp_details + name +"-CC-"+str(i)+"-All");
    
    save(valores, subdir_tmp + name +"-CC");

    return valores;

#
#
def getCC(G_original, graphs, name):
    size_x = len(graphs);
    size_y = 0;
    if size_x > 0:
        size_y = len(graphs[0]);
    ...
    # calcular valores
    logging.debug("starting "+name+" Closeness Centrality..." );
    logging.debug("   size "+name+" "+str(size_x)+" x "+str(size_y));
    valores = getCCValues(graphs, name);
    logging.debug("   ...done!");
    # variables
    valores_totales = [];
    ...
    save(valores_totales, subdir_tmp + name +"-CC-totals");
    save(valores_rms, subdir_tmp + name +"-CC-RMS");
